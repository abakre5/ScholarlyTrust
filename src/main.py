        ...
        if st.button("Check Journal"):
            if issn_input:
                ...
                if metadata and isinstance(metadata, dict):
                    ...
                    st.write(f"**Publisher**: {str(publisher)}")
        ...
        if st.button("Check Paper"):
            if paper_input:
                ...
                if metadata and isinstance(metadata, dict):
                    ...
                    st.write(f"**Cited By Count (per year)**: {str(round(float(cited_by_count), 2)) if isinstance(cited_by_count, (int, float)) else 'N/A'}")
                    ...
                    external_recognition = metadata.get('external_recognition', 'None')
                    st.write(f"**Average Publication Count (normalized)**: {str(avg_author_publications)}")
                    st.write(f"**Average Cited By Count (normalized)**: {str(avg_author_cited_by_count)}")
                    ...
                    st.write(f"**External Recognition (Citations/Co-authorship)**: {str(external_recognition)}")
